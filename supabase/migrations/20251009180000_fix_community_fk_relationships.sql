-- Fix Community Section Foreign Key Relationships
-- Resolves 400 errors for community_content and words table embeds

BEGIN;

-- 1) Sanitize invalid UUIDs to NULL
...
-- 2) Ensure column types are UUID (idempotent)
DO $$ BEGIN
  IF EXISTS (
...
-- 3) Null orphan references (no matching rows in profiles/languages)
...
-- 4) Drop and recreate FKs with exact expected names
...
-- 6) Public read RLS (drop + create for idempotency)
...
-- 7) Reload PostgREST schema cache to reflect changes immediately
...
/*
-- Check constraints exist and point to the right tables
...
-- Check policies
SELECT polname, schemaname, tablename, cmd
FROM pg_policies
WHERE schemaname = 'public' AND tablename IN ('languages','profiles');
*/